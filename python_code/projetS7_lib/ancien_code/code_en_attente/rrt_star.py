...
class RRT_STAR():
    ...
    def nearest(self, pose):
        ...
        for i in range(len(self.vertices)):
            ...
            if d1 < d and self.is_safe(pose, self.vertices[i]):
                d = d1
    ...
    def is_safe(self, pose0, pose1):
        ...
        centroid0 = self.world.get_centroid(robot0)
        centroid1 = self.world.get_centroid(robot1)
        #will this really do it though ?
        link = LineString([tuple(centroid0), tuple(centroid1)])
        #safe_passage = True
    ...
    def limit_edge(self, pose):
        parent, d = self.nearest(pose)
        if self.max_edge_length > self.distance(parent, pose):
            return pose
        else:
            x, y = int(rd.uniform(-self.max_edge_length,self.max_edge_length)), int(rd.uniform(-self.max_edge_length, self.max_edge_length))
            return [parent[0] + x, parent[1] + y, pose[2]]
    ...
    def search(self):
        if self.ready:
            ...
            while i < self.iterations_limit:
                new_pose = [int(rd.uniform(1, self.radius)), int(rd.uniform(1, self.radius)), 2*np.pi*rd.random()]
                ...
                if not self.world.possible_pose(new_pose):
                    ...
                    continue
                #print("yes")
                if not self.is_safe(parent_pose, new_pose):
                    continue
                #Now assuming the generated pose is possible
                self.vertices.append(new_pose)
                parent_index = self.vertices.index(parent_pose)
                #Here we check if the transition is possible
