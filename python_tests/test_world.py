...
def test_world_tuto():
    ...
    robot_tuto = {'relative_robot_polygon': [
                    [-0.5, -0.5],
                    [0.5, -0.5],
                    [0.5, 0.5],
                    [0, 0.75],
    ...
    stream = open(WORLDS_DIR + 'world_tuto.yaml', 'r')
    load_dict = yaml.load(stream)
    stream.close()
    stream = open(ROBOTS_DIR + 'robot_arrow.yaml', 'r')
    robot_load_dict = yaml.load(stream)
    stream.close()
    print("\n\nTest if world_tuto.yaml is written in the good way :")
    print(world_tuto == load_dict and robot_tuto == robot_load_dict)
    print("\n")
    print(load_dict)
    print(robot_load_dict)

    print("\n")
    relative_robot_polygon = Polygon(robot_load_dict['relative_robot_polygon'])
